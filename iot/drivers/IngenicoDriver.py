...
# Manager.load_drivers, the log levels that get applied to the odoo import hierarchy
# won't apply here. This means DEBUG level messages will not display even if specified
# and INFO messages will show even if the log level is configured to be ERROR at the
# odoo-bin level. In order to work around this, it's possible to uncomment this line
# and set the desired level directly for this module.
...
class IngenicoDriver(Driver):
    ...
    def _cleanup_globals(self):
        # Deletes the iot_devices entry which holds a reference to this IngenicoDriver
        # thread. This needs to happen before socket_devices to avoid a race condition
        # with the Manager thread. If socket_devices is deleted first, the Manager will
        # try to delete the corresponding iot_devices entry.
        super().disconnect()

        # Deleting the socket_devices entry will close the socket if it was holding the
        # only remaining reference to the underlying file descriptor. If somebody
        # else is still using it (like a blocking recv call in IngenicoDriver's run
        # method), the socket will not close unless explicitly shut down. That's why
        # `_cleanup_globals` is private and should only be called by IngenicoDriver
        # itself.
        ...
        try:
            ...
            while True:
                ...
                if msg and msg.magic == b'P4Y-ECR!':
                    ...
                    _logger.info("Terminating due to invalid message")
                    self._cleanup_globals()
                    break
                event_manager.device_changed(self)
        except Exception:
            _logger.info("Terminating due to exception")
            self._cleanup_globals()
            _logger.error(format_exc())
