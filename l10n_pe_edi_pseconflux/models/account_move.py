...
class AccountMove(models.Model):
                        ...
                        if response[1].get("emision_aceptada")==True:
                            ...
                            if response[1].get("enlace_del_cdr"):
                                xml_attach = ir_attach.create({
                                    "name":"R-%s-%s-%s.zip" % (move.company_id.vat, response[1].get('tipo_de_comprobante'), response[1].get('nombre')),
            ...
            if move.type in ['out_invoice', 'out_refund'] and move.journal_id.type == 'sale' and not move.l10n_pe_dte_conflux_uid:
                ...
                if response[0]:
                    if response[1].get("status")=="success":
                        ...
                        if response[1]["success"]["data"].get("emision_rechazada", False):
                            _invoice.update({
                                "l10n_pe_dte_status": 'rejected',
                                "l10n_pe_dte_status_response": '%s - %s' % (response[1]["success"]["data"].get('sunat_description', ''), response[1]["success"]["data"].get('sunat_note', '')) if has_sunat_notes else '',
                            })
    ...
    def _l10n_pe_prepare_dte_conflux(self):
        ...
        if base_dte.get('good_receipt', False):
            conflux_dte['conformidad_recepcion'] = base_dte.get('good_receipt','')
