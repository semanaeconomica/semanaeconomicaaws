...
class HrImportWdWizard(models.TransientModel):
	...
	def get_wd_template(self):
		...
		return self.env['popup.it'].get_file('Plantilla Worked Days.xlsx', base64.encodestring(b''.join(f.readlines())))
	...
	def get_input_template(self):
		...
		return self.env['popup.it'].get_file('Plantilla Inputs.xlsx', base64.encodestring(b''.join(f.readlines())))
