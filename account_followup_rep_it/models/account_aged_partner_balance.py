...
class report_account_aged_partner(models.AbstractModel):
	...
	def _get_lines(self, options, line_id=None):
		aml_dict = super(report_account_aged_partner,self)._get_lines(options, line_id)
		for i in aml_dict:
			level = i.get('level')
			if level == 4:
				acc = self.env['account.move.line'].browse(i.get('id'))
				i['name'] = acc.move_id.ref if acc.move_id.ref else ''
...
class ReportAgedPartnerBalance(models.AbstractModel):
	...
	def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
		...
		for i in range(5):
			...
			# Add filter reconciled -> account.move.line BO
			move_lines = move_lines.filtered(lambda l: l.reconciled == False)
		...
		# Add filter reconciled -> account.move.line BO
		for line in self.env['account.move.line'].browse(aml_ids).filtered(lambda l: l.reconciled == False):
			partner_id = line.partner_id.id or False
