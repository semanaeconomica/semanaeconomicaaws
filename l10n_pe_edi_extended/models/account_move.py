...
class AccountMove(models.Model):
    ...
    l10n_pe_dte_operation_type = fields.Selection(
        ...
        compute='_compute_l10n_pe_dte_operation_type', copy=True)
    ...
    l10n_pe_dte_good_receipt = fields.Char(string='Good Receipt')
    ...
    # === Amount fields ===
    l10n_pe_dte_amount_subtotal = fields.Monetary(string='Subtotal',store=True, readonly=True, compute='_compute_dte_amount', track_visibility='always', help='Total without discounts and taxes')
    l10n_pe_dte_amount_discount = fields.Monetary(string='Discount', store=True, readonly=True, compute='_compute_dte_amount',track_visibility='always')    
    l10n_pe_dte_amount_base = fields.Monetary(string='Base Amount', store=True, readonly=True, compute='_compute_dte_amount', track_visibility='always', help='Total with discounts and before taxes')
    l10n_pe_dte_amount_exonerated = fields.Monetary(string='Exonerated  Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_free = fields.Monetary(string='Free Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_unaffected = fields.Monetary(string='Unaffected Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_exportation = fields.Monetary(string='Exportation Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_prepaid = fields.Monetary(string='Prepaid Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_untaxed = fields.Monetary(string='Total before taxes', store=True, compute='_compute_dte_amount', track_visibility='always', help='Total before taxes, all discounts included')
    l10n_pe_dte_global_discount = fields.Monetary(string='Global discount', store=True, readonly=True, compute='_compute_dte_amount',track_visibility='always')  
    l10n_pe_dte_amount_in_words = fields.Char(string="Amount in Words", compute='_l10n_pe_dte_amount_in_words')
    l10n_pe_dte_amount_perception_base = fields.Float(string='Perception Base', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_perception_percentage = fields.Float(string='Perception Percentage', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_perception = fields.Float(string='Perception Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    ...
    l10n_pe_dte_amount_icbper = fields.Monetary(string='ICBPER Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_igv = fields.Monetary(string='IGV Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_isc = fields.Monetary(string='ISC Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_others = fields.Monetary(string='Other charges', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_is_einvoice = fields.Boolean('Is E-invoice')
    l10n_pe_dte_amount_total = fields.Monetary(string='Total Amount', store=True, compute='_compute_dte_amount', track_visibility='always')
    l10n_pe_dte_amount_total_with_perception = fields.Float(string='Total Amount With Perception', store=True, compute='_compute_dte_amount', track_visibility='always')
    ...
    def _compute_dte_amount(self):
        for move in self:
            ...
            move.l10n_pe_dte_amount_igv = sum([x[1] for x in move.amount_by_group if x[0] == 'IGV'])
            #move.l10n_pe_dte_amount_isc = sum([x[1] for x in move.amount_by_group if x[0] == 'ISC'])
            ...
            move.l10n_pe_dte_amount_icbper = sum([x[1] for x in move.amount_by_group if x[0] == 'ICBPER'])
            move.l10n_pe_dte_amount_others = sum([x[1] for x in move.amount_by_group if x[0] == 'OTROS'])
            move.l10n_pe_dte_amount_perception = sum([x[1] for x in move.amount_by_group if x[0] == 'PER'])
            move.l10n_pe_dte_amount_perception_base = sum([x[2] for x in move.amount_by_group if x[0] == 'PER'])
            ...
            #move.l10n_pe_dte_amount_free = sum([x[2] for x in move.amount_by_group if x[0] == 'GRA'])
    ...
    def _l10n_pe_prepare_dte(self):
        ...
        document = {
            ...
            'good_receipt':self.l10n_pe_dte_good_receipt,
    ...
    def _get_name_invoice_report(self, report_xml_id):
        ...
        if self.company_id.country_id.code == 'PE' and self.l10n_pe_dte_is_einvoice:
            custom_report = {
                'account.report_invoice_document_with_payments': 'l10n_pe_edi_extended.report_einvoice_document',
                'account.report_invoice_document': 'l10n_pe_edi_extended.report_einvoice_document',
            }
...
class AccountMoveLine(models.Model):
    ...
    l10n_pe_dte_price_base = fields.Monetary(string='Subtotal without discounts', digits=(9,10), store=True, readonly=True, currency_field='always_set_currency_id', help="Total amount without discounts and taxes")
    l10n_pe_dte_price_unit_excluded = fields.Float(string='Price unit excluded', digits=(9,10), store=True, readonly=True, help="Price unit without taxes")
    l10n_pe_dte_price_unit_included = fields.Float(string='Price unit IGV included', digits=(9,10), store=True, readonly=True, help="Price unit with IGV included")
    l10n_pe_dte_amount_discount = fields.Monetary(string='Amount discount before taxes', store=True, readonly=True, currency_field='always_set_currency_id', help='Amount discount before taxes')
    l10n_pe_dte_amount_free = fields.Monetary(string='Amount free', digits=(9,10), store=True, readonly=True, currency_field='always_set_currency_id', help='amount calculated if the line id for free product')
    l10n_pe_dte_free_product = fields.Boolean('Free', store=True, readonly=True, default=False, help='Is free product?')
    l10n_pe_dte_igv_amount = fields.Monetary(string='IGV amount',store=True, readonly=True, currency_field='always_set_currency_id', help="Total IGV amount")
    l10n_pe_dte_isc_amount = fields.Monetary(string='ISC amount',store=True, readonly=True, currency_field='always_set_currency_id', help="Total ISC amount")
    l10n_pe_dte_icbper_amount = fields.Monetary(string='ICBPER amount',store=True, readonly=True, currency_field='always_set_currency_id', help="Total ICBPER amount")
    #l10n_pe_dte_per_base = fields.Monetary(string='PERCEPTION base',store=True, readonly=True, currency_field='always_set_currency_id', help="Total PERCEPTION base")
    #l10n_pe_dte_per_amount = fields.Monetary(string='PERCEPTION amount',store=True, readonly=True, currency_field='always_set_currency_id', help="Total PERCEPTION amount")
    ...
    l10n_pe_dte_advance_amount = fields.Monetary(string='Advance amount',store=True, readonly=True, currency_field='always_set_currency_id', help="Total Advance amount")
