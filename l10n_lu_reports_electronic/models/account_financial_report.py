    ...
    def _get_lu_electronic_report_values(self, options):
        ...
        for line in filter(lambda l: 'financial_group_line_id' not in l and isinstance(l.get('id'), int), lines):
            ...
            if len(split_line_code) > 2:
                ...
                _report_useful_fields(columns[0].get('no_format_name', 0.0), split_line_code[2], parent_code, required)
                # previous year balance
                _report_useful_fields(columns[1].get('no_format_name', 0.0), str(int(split_line_code[2]) + 1), parent_code and str(int(parent_code) + 1), required)
