...
class ExchangeDiffConfigLine(models.Model):
	...
	def _check_unique_exchange(self):
		self.env.cr.execute("""select period_id from exchange_diff_config_line where line_id = %s and period_id = %s""" % (str(self.line_id.id),str(self.period_id.id)))
		res = self.env.cr.dictfetchall()
		if len(res) > 1:
			raise UserError(u"Ya existe el periodo "+self.period_id.code+u" en los Tipos de Cambio de Cierre para esta Compañía")