...
class AccountExchangeDocumentRep(models.TransientModel):
	...
	def get_report(self):
		self.env.cr.execute("""
			CREATE OR REPLACE view account_exchange_document_book as ("""+self._get_sql_report(self.fiscal_year_id.name,self.period.code,self.company_id.id)+""")""")
	...
	def do_invoice(self):
		...
		sql = """SELECT smd.*,ei.id AS type_document_id FROM get_saldos_me_documento_final('%s','%s',%s) smd
				 LEFT JOIN einvoice_catalog_01 ei ON ei.code = smd.td_sunat""" % (str(self.fiscal_year_id.name),str(self.period.code),str(self.company_id.id))
	...
	def _get_sql_report(self,fiscal_year,period,company_id):
		...
		sql = """SELECT 
				...
				FROM get_saldos_me_documento_final('%s','%s',%s) vst
