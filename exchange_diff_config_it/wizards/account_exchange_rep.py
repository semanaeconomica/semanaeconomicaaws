...
class AccountExchangeRep(models.TransientModel):
	...
	def get_report(self):
		self.env.cr.execute("""
			CREATE OR REPLACE view account_exchange_book as ("""+self._get_sql_report(self.fiscal_year_id.name,self.period.code,self.company_id.id)+""")""")
	...
	def do_invoice(self):
		...
		sql = """SELECT * FROM get_saldos_me_global_final('%s','%s',%s)""" % (str(self.fiscal_year_id.name),str(self.period.code),str(self.company_id.id))
	...
	def _get_sql_report(self,fiscal_year,period,company_id):
		...
		sql = """SELECT 
				...
				FROM get_saldos_me_global_final('%s','%s',%s) gsm
