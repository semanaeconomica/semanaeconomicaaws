...
class AccountMove(models.Model):
    ...
    def find_partner_id_with_vat(self, vat_number_ocr):
        return self.env["res.partner"].search([("vat", "=ilike", vat_number_ocr), *self._domain_company()], limit=1)
    ...
    def get_form_context_manager(self):
        self_ctx = self.with_context(default_type=self.type) if 'default_type' not in self._context or self._context['default_type'] is None else self
