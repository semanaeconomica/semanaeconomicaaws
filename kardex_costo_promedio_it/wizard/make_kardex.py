		...
		for self in selfs:
			...
			if self.state == 'done':				
				self.env.cr.execute("""
						...
						from get_kardex_v(20150101,20500101,'{0,""" +str(self.product_id.id)+ """}'::INT[], (select array_agg(id) from stock_location) ) 
	...
	def do_rebuild(self):
		for i in self.env['product.product'].search([]):		
			self.env.cr.execute("""
					 select 
					CASE WHEN sum(ingreso)!= 0 THEN round(sum(debit),6)/sum(ingreso)    ELSE 0 END
					from get_kardex_v(20150101,20500101,'{0,""" +str(i.id)+ """}'::INT[], (select array_agg(id) from stock_location) ) 
	...
	def procesar(self):
		...
		for i in self.detalle_ids:	
			self.env.cr.execute("""
					 select 
					CASE WHEN sum(ingreso)!= 0 THEN round(sum(debit),6)/sum(ingreso)    ELSE 0 END
					from get_kardex_v(20150101,20500101,'{0,""" +str(i.producto_rel.id)+ """}'::INT[], (select array_agg(id) from stock_location) ) 
