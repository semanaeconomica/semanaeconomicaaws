...
class HrUtilitiesIt(models.Model):
	...
	def _get_sql_utlities(self,rule_total_income,wd_dtrab,wd_falt):
		sql = """
			SELECT coalesce(he.identification_id, '') as employee_document, he.name as employee, he.id as employee_id,had.name as distribution_id, T1.sueldos, T2.dias FROM hr_contract hc
			LEFT JOIN hr_analytic_distribution had on had.id = hc.distribution_id
			LEFT JOIN hr_employee he ON he.id = hc.employee_id
			LEFT JOIN (
			SELECT hp.employee_id,sum(hpl.total) as sueldos FROM hr_payslip_line hpl
			...
			WHERE to_char(hp.date_from::timestamp with time zone, 'yyyy'::text) = '{year}' AND
				...
				hpl.salary_rule_id = {salary_rule_id} AND
				hp.company_id = {company_id}
			GROUP BY hp.employee_id
			) T1 ON T1.employee_id = hc.employee_id
			LEFT JOIN (
			
			SELECT t5.employee_id,t5.dias -t6.dias as dias from(
			
			SELECT t1.employee_id,t1.dias - t2.dias as dias  FROM(
			SELECT hp.employee_id,sum(hpwd.number_of_days) as dias FROM hr_payslip_worked_days hpwd
			...
			) T2 ON T2.employee_id = hc.employee_id
			WHERE T1.sueldos is not null AND T2.dias is not null
			AND hc.company_id = {company_id}
	...
	def get_excel_utilidades(self):
		...
		return self.env['popup.it'].get_file('Utilidades -.xlsx',base64.encodestring(b''.join(f.readlines())))
		...
		data_title = [[Paragraph('<strong>LIQUIDACIÓN DE DISTRIBUCIÓN DE UTILIDADES DEL EJERCICIO %s</strong>' % (self.main_id.fiscal_year_id.name if self.main_id.fiscal_year_id else ''),style_title)],
						[Paragraph('%s IDENTIFICADA CON RUC NRO %s  DEBIDAMENTE REPRESENTADA POR %s EN SU CALIDAD DE EMPLEADOR Y EN CUMPLIMIENTO DE LO DISPUESTO POR EL D.L. 892 Y EL D.S. \
							NRO. 009-98-TR, DEJA CONSTANCIA DE LA DETERMINACION, DISTRIBUCION Y PAGO DE LA PARTICIPACION EN LAS UTILIDADES DEL TRABAJADOR %s' % (
								...
								MainParameter.legal_representative.upper(),
