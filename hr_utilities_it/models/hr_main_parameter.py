...
class HrMainParameter(models.Model):
	...
	rule_total_income = fields.Many2one('hr.salary.rule', string='Regla salarial para Total de Ingresos')
	...
	wd_falt = fields.Many2many('hr.payslip.worked_days.type', 'wd_falt_main_parameter_rel', 'main_parameter_id', 'wd_id', string='Worked Days Faltas u otras')
	hr_input_for_results = fields.Many2one('hr.payslip.input.type', string='Input para Resultados')
	...
	def check_utility_values(self):
		if not self.rule_total_income or \
		   ...
		   not self.wd_falt or \
		   not self.legal_representative or \
