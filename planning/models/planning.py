...
class Planning(models.Model):
    ...
    allocated_percentage = fields.Float("Allocated Time (%)", default=100, help="Percentage of time the employee is supposed to work during the shift.", group_operator="avg")
        ...
        if any(key in ('repeat', 'repeat_type', 'repeat_until', 'repeat_interval') for key in values):
            ...
            for slot in self:
                if slot.recurrency_id and values.get('repeat') is None:
                    repeat_until = False
                    if values.get('repeat_type') == 'until':
                        repeat_until = datetime.combine(values.get('repeat_until'), datetime.max.time())
                        repeat_until = repeat_until.replace(tzinfo=pytz.timezone(slot.company_id.resource_calendar_id.tz or 'UTC')).astimezone(pytz.utc).replace(tzinfo=None)
                    recurrency_values = {
