...
class MrpProductionSchedule(models.Model):
    ...
    def get_impacted_schedule(self, domain=False):
        ...
        def _used_in_bom(products, related_products):
            ...
            produced_products = self.env['product.product']
            boms_lines = products.bom_line_ids
            for line in boms_lines:
                if line.bom_id.product_id:
                    produced_products |= line.bom_id.product_id
                else:
                    produced_products |= line.bom_id.product_tmpl_id.product_variant_ids.filtered(lambda p: not line._skip_bom_line(p))
            produced_products -= related_products
            related_products |= produced_products
            return _used_in_bom(produced_products, related_products)
        ...
        def _use_boms(products, related_products):
            ...
            for product in products:
                ...
                components |= boms.bom_line_ids.filtered(lambda l: not l._skip_bom_line(product)).product_id
    ...
    def _get_indirect_demand_tree(self):
        ...
        def _get_product_tree(product, ratio):
            ...
            for line in product_boms.bom_line_ids.filtered(lambda l: not l._skip_bom_line(product)):
