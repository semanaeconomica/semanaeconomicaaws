...
class TestMpsMps(common.TransactionCase):
    ...
    def test_replenish_with_variants(self):
        legs_attribute = self.env['ir.model.data'].xmlid_to_object('product.product_attribute_1')
        product_tmpl = self.env['product.template'].create({
            'name': 'Super Product',
            'type': 'product',
            'attribute_line_ids': [(0, 0, {
                'attribute_id': legs_attribute.id,
                'value_ids': [[6, 0, legs_attribute.value_ids[:2].ids]]
            })]
        })
        product01, product02 = product_tmpl.product_variant_ids
        ...
        self.env['mrp.bom'].create({
            ...
            'bom_line_ids': [(0, 0, {
                ...
                'product_qty': 1,
                'product_uom_id': 1,
                'bom_product_template_attribute_value_ids': [(4, product01.product_template_attribute_value_ids.id)],
            }), (0, 0, {
                'product_id': component02.id,
                'product_qty': 1,
                'product_uom_id': 1,
                'bom_product_template_attribute_value_ids': [(4, product02.product_template_attribute_value_ids.id)],
            })]
        })

        product01_mps, product02_mps, component01_mps, component02_mps = self.env['mrp.production.schedule'].create([{
            'product_id': p.id,
            'warehouse_id': self.warehouse.id,
        } for p in [product01, product02, component01, component02]])
        self.assertEqual(product01_mps.get_impacted_schedule(), component01_mps.ids)
        self.assertEqual(component01_mps.get_impacted_schedule(), product01_mps.ids)
        self.assertEqual(product02_mps.get_impacted_schedule(), component02_mps.ids)
        self.assertEqual(component02_mps.get_impacted_schedule(), product02_mps.ids)

        product01_mps.set_replenish_qty(0, 2)
        product02_mps.set_replenish_qty(0, 3)
        product01_res, product02_res, component01_res, component02_res = (product01_mps + product02_mps + component01_mps + component02_mps).get_production_schedule_view_state()
        self.assertEqual(product01_res['forecast_ids'][0]['replenish_qty'], 2.0)
        self.assertEqual(product02_res['forecast_ids'][0]['replenish_qty'], 3.0)
        self.assertEqual(component01_res['forecast_ids'][0]['replenish_qty'], 2.0)
        self.assertEqual(component02_res['forecast_ids'][0]['replenish_qty'], 3.0)
