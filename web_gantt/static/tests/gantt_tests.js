...
QUnit.module('Views', {
        ...
        this.data = {
            tasks: {
                ...
                records: [
                    ...
                    { id: 8, name: 'Task 8', start: '2020-10-24 06:30:12', stop: '2020-10-24 18:29:59', stage: 'in_progress', stage_id: 1, project_id: 2, user_id: 2, color: 10, progress: 80},
                ],
            },
            projects: {
                fields: {
    ...
    QUnit.test('move a pill in the same row (with different timezone)', async function (assert) {
        ...
        if(!dstDate) {
            ...
            // as part of a bugfix...)
            dstDate = moment('2020-10-24 12:00:00');
        }

        var initialDate = dstDate;
        var taskStart = dstDate.clone().hour(6).minute(30).utc();
        this.data.tasks.records[7].start = taskStart.format('YYYY-MM-DD HH:mm:ss');
        this.data.tasks.records[7].stop = taskStart.clone().local().hour(17).utc().format('YYYY-MM-DD HH:mm:ss');
        var gantt = await createView({
            View: GanttView,
            model: 'tasks',
            data: this.data,
            ...
            mockRPC: function (route, args) {
                if (args.method === 'write') {
                    assert.deepEqual(args.args[0], [8],
                        "should write on the correct record");
                    assert.equal(6, moment.utc(args.args[1].start).local().hour(),
                        "start date should have the same local time as original")
                    assert.equal(17, moment.utc(args.args[1].stop).local().hour(),
    ...
    QUnit.test("plan dialog initial domain has the action domain as its only base",
        async function (assert) {
            ...
            await testUtils.dom.click($('.o_control_panel .o_filters_menu a:contains(project_one)'));
            assert.verifySteps(["project_id,=,1,start,<=,2018-12-31 23:59:59,stop,>=,2018-12-01 00:00:00"]);
