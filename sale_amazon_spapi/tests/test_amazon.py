...
class TestAmazon(TransactionCase):
    ...
    @mute_logger('odoo.addons.sale_amazon_spapi.models.amazon_account')
    @mute_logger('odoo.addons.sale_amazon_spapi.models.sale_order')
    def test_sync_cancellations(self):
        ...
        with patch(
            ...
            self.assertEqual(
                mock.call_count,
                1,
                msg="An order acknowledgement feed should be sent to Amazon for each canceled "
                    "order.",
            )
    ...
    def test_get_amazon_offer_search(self):
        ...
        order_data = OPERATIONS_RESPONSES_MAP['getOrders']['payload']['Orders'][0]
        self.assertTrue(self.account._get_offer(order_data, 'TEST'))
    ...
    def test_get_amazon_offer_creation(self):
        ...
        offers_count = self.env['amazon.offer'].search_count([])
        order_data = OPERATIONS_RESPONSES_MAP['getOrders']['payload']['Orders'][0]
        offer = self.account._get_offer(order_data, 'TEST', marketplace_api_ref='ATVPDKIKX0DER')
