...
from odoo.addons.sale_amazon.models.amazon_account import MARKETPLACES_WITH_TAX_INCLUDED
from odoo.addons.sale_amazon_spapi import const, utils as amazon_utils
from odoo.addons.sale_amazon_spapi.controllers.onboarding import compute_oauth_signature
...
class AmazonAccount(models.Model):
    ...
    def _create_order_from_data(self, order_data):
        ...
        return self.env['sale.order'].with_context(
            mail_create_nosubscribe=True, allowed_company_ids=self.company_id.ids
        ).create(order_vals)
    ...
    def _prepare_order_lines_values(self, order_data, currency, fiscal_pos, shipping_product):
        ...
        for item_data in items_data:
            ...
            offer = self._get_offer(order_data, sku, marketplace_api_ref=marketplace_api_ref)
            ...
            original_subtotal = sales_price - tax_amount \
                if MARKETPLACES_WITH_TAX_INCLUDED.get(marketplace_api_ref, False) else sales_price
    ...
    @staticmethod
    def _get_marketplace_api_ref(_order_data, *args, **kwargs):
        """ Override of sale_amazon to extract `marketplace_api_ref` from the kwargs rather than
        from _order_data. """
        return kwargs.get('marketplace_api_ref')
