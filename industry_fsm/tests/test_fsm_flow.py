...
class TestFsmFlow(common.TransactionCase):
    ...
    def test_fsm_flow(self):
        ...
        #If there aren't accounting sale journals the task shouldn't be invoiceable.

        journal = self.env['account.journal'].search([])
        if not journal:
            with self.assertRaises(UserError, msg='If there are not accounting sale journals the task should not be invoiceable'):
