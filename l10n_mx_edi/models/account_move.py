...
class AccountMoveLine(models.Model):
    ...
    def _auto_init(self):
        for field in ["l10n_mx_edi_tariff_fraction_id", "l10n_mx_edi_umt_aduana_id"]:
            if not column_exists(self.env.cr, "account_move_line", field):
                create_column(self.env.cr, "account_move_line", field, "int4")
    ...
    def _l10n_mx_edi_get_valor_usd(self, usd, currency, company_id, date):
        self.ensure_one()
        lines = self.move_id.invoice_line_ids.filtered(lambda l: l.product_id == self.product_id)
