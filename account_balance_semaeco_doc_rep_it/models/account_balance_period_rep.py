...
class AccountBalancePeriodRep(models.TransientModel):
	...
	def get_report(self):
		...
		sql = """
			CREATE OR REPLACE view account_balance_period_book as (SELECT a1.*, CASE WHEN aml.expected_pay_date IS NOT NULL THEN aml.expected_pay_date ELSE a1.fecha_ven END AS expected_pay_date,
			am.manage_comment	
			 FROM get_saldos('%s','%s',%s,%s) a1
				...
				LEFT JOIN account_move am on a1.move_id = am.id
	...
	def get_excel(self):
		...
		HEADERS = ['PERIODO','FEC CON','LIBRO','VOUCHER','TDP','RUC','PARTNER',
		'TD','NRO COMP','FEC DOC','FEC VEN','F. CANC. APROX.','CUENTA',u'COMENT. GESTIÓN',
		'DEBE','HABER','SALDO MN','SALDO ME']
		...
		for line in self.env['account.balance.period.book'].search([]):
			...
			worksheet.write(x,13,line.manage_comment if line.manage_comment else '',formats['especial1'])
		...
		widths = [7,10,10,10,4,11,40,4,10,10,10,10,10,10,12,12,12,12]
	...
	def get_pdf(self):
		...
		for fila in self.env['account.balance.period.book'].search([]):
			...
			manage_comment = (fila['manage_comment']) if fila['manage_comment'] else ''
